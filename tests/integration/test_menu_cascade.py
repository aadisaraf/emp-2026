...
def test_scenario_1_every_recipe_using_a_recalled_ingredient_is_listed(loaded, run_id):
    ...
    pulled = {r["normalized_description"] for r in loaded.execute(
        """SELECT i.normalized_description FROM matches m
    ...
    for norm in pulled:
        for row in loaded.execute(
            ...
            expected.add(row["recipe_id"])
    actual = {r["recipe_id"] for e in entries for r in e["recipes"]}
    assert actual == expected


def test_scenario_2_dates_carry_planned_counts_labelled_planned(loaded, run_id):
    ...
    for date, recipe_id, planned in summary["service_days"]:
        row = loaded.execute(
            """SELECT planned_meals FROM service_days
                WHERE date = ? AND recipe_id = ?""", (date, recipe_id)).fetchone()
    ...
    assert summary["planned_meals"] == sum(d[2] for d in summary["service_days"])
    assert len(summary["service_days"]) == len({(d[0], d[1])
                                                for d in summary["service_days"]})

    # The label reaches the page, not just the dictionary.
    page = TestClient(app).get("/impact")
    assert page.status_code == 200
    assert "planned, not served" in page.text


def test_scenario_3_a_real_substitute_is_proposed_and_names_its_components(loaded, run_id):
    proposals = substitute.proposals_for(loaded, run_id, cascade.cascade(loaded, run_id))
    ...
    for p in offered:
        ...
        # And the substitute is genuinely cookable and un-pulled in this kitchen.
        assert p["recipe_id"] in substitute.clean_candidates(loaded, run_id)


def test_scenario_4_no_substitute_is_a_named_component_not_a_shrug(loaded, run_id):
    proposals = substitute.proposals_for(loaded, run_id, cascade.cascade(loaded, run_id))
    ...
    for p in declined:
        ...
        # The reason names the component AND how many candidates were checked,
        # so "none" reads as a search that happened rather than one that did not.
        assert p["reason"] and all(u in p["reason"] for u in p["unmet"])
        assert str(p["candidates_checked"]) in p["reason"]
        # The decline carries no substitute of any kind -- not even a partial one.
        assert "recipe_id" not in p and "name" not in p


def test_scenario_4_no_approximate_substitute_is_ever_offered(loaded, run_id):
    """The property behind scenario 4: containment, never a closest match."""
    components = substitute._components(loaded)
    for p in substitute.proposals_for(loaded, run_id, cascade.cascade(loaded, run_id)):
...
def test_scenario_5_the_revised_menu_prints(loaded):
    ...
    for heading in ("Menu", "Meals with no substitute"):
...
def test_held_lines_are_excluded_but_the_exclusion_is_stated(loaded, run_id):
    ...
    every = cascade.cascade(loaded, run_id, statuses=("PULL", "HELD"))
