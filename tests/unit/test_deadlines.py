...
from pullsheet.rollup import deadlines
...
def test_both_clocks_exist_and_run_from_receipt_not_report_date(loaded):
    clocks = {c["key"]: c for c in deadlines.clocks(loaded, _received(loaded))}
...
def test_at_25_hours_one_clock_has_overrun_and_the_other_has_not(loaded):
    """T065's verify, asserted at the instant it names."""
    clocks = {c["key"]: c for c in deadlines.clocks(loaded, _received(loaded) + timedelta(hours=25))}
...
def test_an_elapsed_deadline_never_hides_or_resets(loaded):
    """FR-053. The overrun keeps growing; it does not wrap, blank, or turn green."""
    received = _received(loaded)
    a = deadlines.clocks(loaded, received + timedelta(hours=30))[0]
    b = deadlines.clocks(loaded, received + timedelta(hours=100))[0]
    assert a["overrun"] and b["overrun"]
    assert b["remaining_hours"] < a["remaining_hours"]
    assert "OVERRUN" in a["text"] and "OVERRUN" in b["text"]
    assert deadlines.clocks(loaded, received + timedelta(days=30)), \
...
def test_no_matches_means_no_clocks(tmp_path):
    """A district with nothing on its sheet is not against a deadline."""
    path = tmp_path / "empty.db"
    db.reset(path)
    conn = db.connect(path)
    corpus.load_snapshots(conn)
    from datetime import datetime, timezone
    assert deadlines.clocks(conn, datetime(2026, 9, 5, tzinfo=timezone.utc)) == []
    conn.close()
