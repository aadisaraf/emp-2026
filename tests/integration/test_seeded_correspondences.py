...
def _identifies(line, seed) -> bool:
    ...
    One location can stock the same product in two lots, and the two lots are
    two different answers -- so the description alone is not an identifier.
    The oracle names a row of the export; this resolves that row to the
    description and lot the sheet actually carries.
    """
