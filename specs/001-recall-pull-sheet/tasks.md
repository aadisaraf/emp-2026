---
description: "Task list for PullSheet — food-recall response for one food-service location"
---

# Tasks: PullSheet — Food-Recall Response for One Food-Service Location
...
> ## Amendment 3 (2026-09-05) — read this before running any Verify command
>
> This file is a **record of what was built**, in the order it was built. Every task below was
> completed against the district model: many locations on one screen, a roll-up, a standing
> monitor. Amendment 3 replaced that model with one deployment per location and one dated run per
> day. See [spec.md](./spec.md) and [plan.md](./plan.md) for what the system is now.
>
> Rewriting the tasks to pretend they always said "location" would falsify the record, so they are
> left as written, with three kinds of edit applied:
>
> - **Withdrawn** tasks — the deliverable no longer exists — are struck through and say why.
> - **Re-homed** tasks — the deliverable exists under a different name — say where it went.
> - **Verify commands** that would now fail have been corrected in place, because a stale command
>   is worse than a stale sentence: someone will run it.
>
> Task ids and FR/SC ids are never renumbered. A tombstone is cheaper than a broken cross-reference.
...
- [X] T007 Author `data/fixtures/inventory_lincoln.csv` — the ~50-line hand-authored inventory export for one location. Must contain: ≥ 3 storage locations; deliberately abbreviated descriptions (`chkn strips froz`, `grnd bf 80/20`, `mozz shred lm`); ≥ 5 rows with no GTIN (produce and USDA commodity foods); ≥ 2 rows with no unit cost; the same product in two storage locations with different lots; lot codes in mismatched formats (`4829-B` where the recall says `LOT 4829B`); one row with a blank quantity; PrimeroEdge-style headers. **Verify**: `python -c "import csv;r=list(csv.DictReader(open('data/fixtures/inventory_lincoln.csv')));print(len(r), len({x['Storage Location'] for x in r}))"` → 56 rows, ≥ 3 storage locations. *(Amendment 3: the `Site` column was dropped from the fixture. A building name is not a place in a kitchen.)* ⚠️ **Over 45 min** — this is authored data whose correctness the whole demo rests on.
  - *Split*: T007a author the header row and 17 rows for the first storage location, fixing the CSV shape; T007b and T007c author the other two in parallel against that shape.
...
- [X] T009 [P] Author the menu fixtures in `data/fixtures/`: `recipes.csv`, `recipe_ingredients.csv`, `recipe_components.csv` (`grain`/`meat_or_alternate`/`fruit`/`vegetable`/`milk`), `service_days.csv` (`date, recipe_id, planned_meals`), transcribed from a real published school menu and labeled `hand-authored`. At least one recipe must use a seeded recalled ingredient, and at least one broken recipe must have **no** viable substitute in stock so FR-041 is demonstrable rather than theoretical. **Verify**: `python scripts/check_fixtures.py --menu` → prints ≥ 1 recipe reachable from `expected_matches.json` and ≥ 1 recipe with an unsatisfiable component.
...
- [X] T014 Append the remaining tables to `pullsheet/schema.sql`: `runs`, `recall_snapshots`, `recipes`, `recipe_ingredients`, `recipe_components`, `service_days`. No table gets a delete path. **Verify**: `sqlite3 /tmp/t.db ".read pullsheet/schema.sql" ".tables"` → exactly 10 table names. *(Amendment 3: `inventory_sources`, `ingest_runs` and `monitor_runs` were three tables describing the same event. They are one table, `runs`, and the count fell from 13 to 10.)*
...
sheet appears — grouped by storage location, most serious class first, uncertain matches HELD
and visible.
...
- [X] T032 [US1] Implement `pullsheet/matching/run.py` — the orchestration: for each inventory record, `generate_candidates` → `build_evidence` → `decide` → insert `matches`, ordered `class_rank, tier_rank, score DESC NULLS LAST, id`. (This module is an addition to the plan's source tree; it exists so that `gate.py` stays pure and orchestration is not smuggled into it.) **Verify**: `python -m pullsheet.match --reason audit && sqlite3 data/pullsheet.db "select tier, status, count(*) from matches group by 1,2"` → only `CONFIRMED|PULL`, `PROBABLE|PULL`, `POSSIBLE|HELD` appear. Any other combination is a gate bypass. *(Amendment 3: `--all` is gone. A delivery is now matched as part of being read, and this command exists only for the other direction — the corpus changed and the inventory did not — where it opens a `rematch` run of its own.)*

### Ingestion — the watched folder (demo centerpiece)

> T033–T037 come before the upload adapter deliberately: the watched folder is the
...
- [X] T034 [P] [US1] Implement `pullsheet/adapters/watched_folder.py::read()` *(amendment 3: now `sftp_drop.py` — a watched folder is what an SFTP drop looks like from inside the building)* — CSV and XLSX (via `openpyxl`) through `column_map`, yielding `NormalizedRecord`. Never drops a row, never invents a value, passes `lot_code` verbatim. **Verify**: `pytest tests/adapters/test_sftp_drop.py -v` → the 56-row fixture yields exactly 56 records; the blank-quantity row yields a record with `quantity` unset and `"quantity"` in `unpopulated`, not a dropped row.
- [X] T035 [US1] Implement the poll loop and archive-on-success in `watched_folder.py`, plus `pullsheet/main.py` starting it alongside uvicorn in one process. A rejected file **stays put** so it remains visible. **Verify**: start the app, `cp data/fixtures/inventory_lincoln.csv data/watched/` → within one poll interval the file is in `data/archive/`, and `sqlite3 data/pullsheet.db "select status, row_count from runs"` → one `ok` row with 56.
- [X] T036 [US1] Implement ingest persistence in `db.py`/`watched_folder.py` — write a `runs` row (channel, delivery reference, business date, row count, parsed, partial, status, rejection reason) and insert `inventory_records` with `unpopulated_fields` and `normalized_description` computed by T017. **Verify**: `pytest tests/integration/test_ingest.py -v` → row counts in `runs` equal rows in `inventory_records` for that run, and every partially-parsed row is present with a non-empty `unpopulated_fields`.
- [X] T037 [US1] Implement identity merge and supersession — `identity_key` from `(storage_location, product_identity, lot_code)` where `product_identity` is `gtin` or else `normalized_description`; matching identities merge with summed quantities and every contributing source row retained in `merged_from`; a later export sets `superseded_by` on the earlier rows without destroying them, and preserves existing `decisions`. **An item absent from one morning's export is superseded by nothing and stays on the sheet** — inventory is superseded by a later export, never by silence. **Verify**: `pytest tests/integration/test_ingest_merge.py -v` → two same-identity rows become one record with summed quantity and both source rows in `merged_from` (SC-014); re-ingesting supersedes rather than duplicating, and a prior `clear_match` decision still resolves.
- [X] T038 [US1] Implement rejection handling (FR-006) — a malformed, empty, or unmapped-column export raises `AdapterRejection`, is recorded in `runs` with the failing file and row or column named, leaves any existing pull sheet intact, and returns HTTP 200 with a visible panel rather than a 4xx a folder poller would swallow. **Verify**: `cp tests/adapters/fixtures/malformed.csv data/watched/` → `/` shows a rejection panel naming file, column, and reason; `/sheet` is unchanged; the file is still in `data/watched/`.

### The pull sheet

- [X] T039 [US1] Write `pullsheet/app.py` — the FastAPI skeleton, `GET /` placeholder, and `GET /api/status` returning `{state, word, detail, run_id, business_date, pull_count, held_count, new_count, deadlines, corpus, run_count}`. **Verify**: `curl -s localhost:8000/api/status | python -m json.tool` → every key present, `corpus[].captured_at` matching the committed snapshot meta. *(Amendment 3: `sites` is gone; `state`/`word`/`new_count` replace it, because the question a single location asks is "what about today", not "which building".)*
...
- [X] T041 [US1] Implement `pullsheet/artifacts/pull_sheet.py` — the query and grouping: by storage location, ordered `class_rank, tier_rank, score DESC NULLS LAST, id`, returning PULL and HELD interleaved in that single order (HELD is never a separate section and never behind a toggle). **Verify**: `pytest tests/integration/test_seeded_correspondences.py tests/unit/test_determinism.py -v` → for the fixture's mixed classes and tiers the order matches the expected sequence exactly, no HELD row is absent, and two fresh databases produce byte-identical orderings.
- [X] T042 [US1] Write `templates/sheet.html` and wire `GET /sheet` and `GET /sheet/{run_id}` — item description (verbatim `raw_description`), quantity with unit, storage location, lot, tier badge, status, recall class, and the triggering value. HELD rows visually distinct. Header carries the location, the run's business date, generation timestamp, corpus source, provenance label, and capture date. A past run renders exactly as it was printed, with its own frozen counts and its own corpus note. Zero matches still renders the sheet stating zero lines matched and against which corpus and date. **Verify**: open `/sheet` → sections per storage location, Class I first, HELD rows distinct at a glance; `/sheet/1` shows the past-run banner; `sqlite3 data/pullsheet.db "delete from matches"` then reload → the sheet still renders with the zero-line statement. ⚠️ **Over 45 min** — this is the demo's primary screen.
  - *Split*: T042a the table markup and grouping loop; T042b the header block, tier badges, and HELD styling.
- [X] T043 [US1] Write `pullsheet/static/print.css` and link it — every column survives standard Letter portrait, provenance labels and the capture date print, storage-location sections break cleanly. **Verify**: print-preview `/sheet` → all columns visible, nothing clipped at the right margin, provenance label present on the printed page (SC-008, FR-035).
...
- [X] T047 [P] [US1] ~~Implement `pullsheet/adapters/paste.py` and `POST /ingest/paste` — the floor.~~ **WITHDRAWN (amendment 3.)** Written, then removed with the adapter. The floor under a scheduled delivery is not a textarea: an export is a file, and a pasted blob invited a shape nothing downstream could honestly read a storage location or a lot code out of. The real floor is `spreadsheet_upload` (T048) for the morning the drop fails. **Verify**: `python -c "import pullsheet.adapters.paste"` → `ModuleNotFoundError`, and `grep -rc 'ingest/paste' pullsheet/` → 0.
...
- [X] T049 [US1] Implement the column-mapping step — when `detect()` returns ambiguous fields, prompt once, then store **the answer to that header** and reuse it silently. **Verify**: upload `headers_adhoc.csv` → mapping prompt appears for the ambiguous header only; submit it; upload a differently-named file with the same ambiguous header → no prompt. *(Amendment 3: the memory stores answers to ambiguous headers, never a whole mapping. Detection runs per file, so a source that changes its export shape is re-read rather than forced into last week's columns.)*
...
- [X] T056 [US2] Implement `pullsheet/menu/cascade.py` — recalled item → recipes using it → scheduled service dates → planned meal count summed from `service_days.planned_meals`. The count is **planned**, never measured, and carries a `hand-authored` provenance label. **Verify**: `pytest tests/integration/test_menu_cascade.py -v` → a seeded recalled ingredient reaches the expected recipe set and the expected date/count rows, with the count equal to the hand-summed fixture value.
- [X] T057 [US2] Implement `pullsheet/menu/substitute.py` — set containment over `recipe_components`. A substitute is proposed only when the candidate's component set contains the broken recipe's required set; otherwise the function returns the **named unmet component**. "No substitute exists" is a proof, not a failure to find one. **Verify**: `pytest tests/integration/test_menu_cascade.py -v` → the satisfiable recipe gets a proposal naming which components it covers; the deliberately unsatisfiable one from T009 returns a named unmet component and **no** approximate proposal.
- [X] T058 [US2] Implement `GET /impact` and `templates/impact.html` — broken items, affected dates, planned counts, proposals or declines, and the revised menu as a printable artifact under `print.css`. **Verify**: open `/impact` → each broken item lists its recipes, dates, and planned counts labeled as planned; print-preview produces a clean revised menu (FR-042). *(Amendment 3: there is no `/menu` page. A menu break is a consequence of a recall, so it lives inside the "what was affected" section next to the money, and the money is shown for every location while the cascade appears only where one runs a meal program.)*
...
- [X] T060 [US3] Implement `pullsheet/artifacts/hold_record.py` and `GET /artifacts/hold` — a listing of every held item with quantity, lot, and storage location, signature and date fields left **blank for a human**. **Verify**: print-preview `/artifacts/hold` → every HELD and PULL line in the latest run is listed, signature and date fields are empty, provenance labels present.
- [X] T061 [US3] Implement `pullsheet/artifacts/credit_claim.py` and `GET /artifacts/credit-claim` — itemized quantity and extended value per line, a total for the location, computed as plain `quantity × unit_cost` arithmetic. Lines with no unit cost appear **quantity-only**, and the claim states that the total excludes them. No price is ever estimated. **Verify**: `pytest tests/integration/test_artifacts.py -v` → the total equals the hand-computed value for the costed lines ($8,862.50 over 24 of 27 lines); the costless fixture items appear with quantity only and are named in the exclusion statement (FR-047).
- [X] T062 [US3] Implement `pullsheet/artifacts/state_report.py` and `GET /artifacts/state-report` — every derivable field pre-filled, every non-derivable field visibly marked as requiring human entry rather than guessed (FR-045). Modelled on USDA FNS recall reporting guidance and labeled `hand-authored`, per the spec's Open Questions interim default for FR-044. **Verify**: open `/artifacts/state-report` → derived fields populated from the database, undeliverable fields rendered with the "requires human entry" marker, zero fields silently blank. ⚠️ **Over 45 min** — form-shaped work with many fields.
  - *Split*: T062a the derived-field query and data assembly; T062b the form template and the unfillable-field marker treatment.
- [X] T063 [US3] Apply the provenance macro to all four artifacts and write `tests/integration/test_artifacts.py` covering US3 acceptance scenarios 1–5. **Verify**: `pytest tests/integration/test_artifacts.py -v` → 5 pass, including an assertion that every artifact response contains a provenance label for each source it drew on (FR-048).

**Checkpoint**: US1–US3 demoable independently.

---

## Phase 6: User Story 4 — The daily status and the deadline clock (Priority: P4)

*(Amendment 3 retitled this phase. It was "District roll-up and deadline clock". The clock is
unchanged; the roll-up became one status word for one location, and moved out of `rollup/`.)*

**Goal**: Today on one screen, with the USDA clock running.

**Independent Test**: With a run finished, the status word is correct and names its reason,
countdowns advance against the recorded receipt time, and a morning with no delivery says so
instead of showing yesterday's sheet as today's answer.

**Depends on**: Phase 3. Uses T031's freshness function.

- [X] T064 [US4] Implement ~~`pullsheet/rollup/status.py`~~ **`pullsheet/runs.py::run_status`** — the location derives exactly one of `never` / `overdue` / `rejected` / `action` / `stale` / `clear`, each carrying the sentence that explains it. **It reads `clear` only when a delivery was successfully processed, that run produced zero PULL lines, and the snapshot in use is inside the 24-hour window.** Status is derived on read, never stored. **Verify**: `pytest tests/unit/test_freshness.py -v` → a run with lines is `action`; a location that has never delivered is `never` with the reason named; a clean run inside the window is `clear`; the same run outside it is `stale`, not `clear`. *(Amendment 3: `unconfirmed` split into `never`, `overdue` and `rejected`. "We have not heard from you", "we heard from you on Tuesday" and "this morning's file could not be read" are three different sentences, and one word for all three was the weakest thing on the old page.)*
- [X] T065 [US4] Implement ~~`pullsheet/rollup/deadlines.py`~~ **`pullsheet/deadlines.py`** — 24-hour distributor-notification and 48-hour inventory-assessment countdowns computed from `recall_records.received_at` against an injected `now`. An elapsed deadline shows the **overrun explicitly** rather than disappearing or resetting. The clocks are scoped to the run, so a past run's sheet prints the clocks as they stood that day. **Verify**: `pytest tests/unit/test_deadlines.py -v` → at `now = received_at + 25h` the 24h clock reads `+1h overrun` and the 48h clock reads `23h remaining`.
- [X] T066 [US4] Implement the dashboard on `GET /` with ~~`templates/rollup.html`~~ **`templates/dashboard.html`** and `static/poll.js` — the status word and its reason, both countdowns, what is new since the previous run, the corpus provenance banner, and a 2-second poll against `/api/status`. **This is the only JavaScript in the project.** `poll.js` also compares `run_id` against the page's own and reloads when a newer run has been finalized — which is what makes the demo work without a click. **Verify**: open `/` → exactly one status word with its reason; drop a file into `data/watched/` without touching the browser → the open tab becomes today's sheet within ~2 seconds.
- [X] T067 [US4] ~~Implement `POST /site/{site}/confirm` — records a `confirm_site_pulled` decision.~~ **WITHDRAWN (amendment 3.)** Written, then removed. "This building is done" was a roll-up gesture: it asserted something about a whole site that nobody had checked line by line. What survives is the per-line action a person actually takes — `POST /match/{id}/confirm-pulled` (T046's sibling) — which claims only what the person in front of the shelf can see. **Verify**: `grep -rc 'confirm_site_pulled' pullsheet/` → 0, and `pytest tests/unit/test_clearing_audit.py::test_only_named_routes_write_a_decision -v` → passes with exactly two decision writers.
- [X] T068 [US4] Complete `tests/unit/test_freshness.py` (SC-013) — inject a `now` 26 hours after capture and assert the status **does not read clear**, that it names the staleness and the age, and that PULL/HELD lines are produced **unchanged**. A run that suppresses lines is a failure: staleness gates one word in the status, not the matcher. The fixture is deliberately a clean export, because with a PULL line on the sheet the word is "items to pull" whatever the corpus's age and every assertion would be vacuous. **Verify**: `pytest tests/unit/test_freshness.py -v` → all pass, including the assertion that the line list is identical stale versus fresh.

**Checkpoint**: US1–US4 demoable independently.

---

## Phase 7: User Story 5 — The daily cycle, unattended (Priority: P5)

*(Amendment 3 retitled this phase. It was "Standing monitor". The monitor had its own table, its
own schedule and its own high-water mark; all three dissolved when the daily run began re-matching
the whole active shelf against the whole corpus every morning. "New" is now a column on a match
row, decided once at the moment the matcher writes it.)*

**Goal**: Nobody has to remember to check, and nobody has to log in on an ordinary morning.

**Independent Test**: Deliver an export, deliver it again the next day unchanged, then deliver one
with a newly recalled item — and confirm that only the third run flags anything, that the second
is recorded anyway, and that a carried-over item is never flagged.

**Depends on**: Phase 3.

- [X] T069 [US5] ~~Implement `pullsheet/monitor.py` with its own schedule and `monitor_runs` table.~~ **RE-HOMED (amendment 3)** into `matching/run.py` + `db.py::previously_matched_pairs`: the daily run re-matches everything, so `is_new` is decided at INSERT by asking whether the *previous good run* produced the same `(item identity, recall)` pair. **The diff is on item identity, never on row id** — a carried-over item is re-recorded under a fresh `inventory_records` id every morning, so an id diff would flag the whole sheet as new every day. Every run is recorded whatever it found (FR-058 — a quiet run is still a run). **Verify**: `pytest tests/integration/test_new_since_last_run.py -v` → 8 pass, including five identical days in a row reporting `[0, 0, 0, 0, 0]`.
- [X] T070 [US5] Render "what is new" on `/` and a `new` flag on each line — an alert *is* a match carrying `is_new = 1`; there is no alerts table. Each names the item, its storage location and the triggering recall record. **Verify**: deliver an export containing a newly recalled item → `/` shows the what's-new block and `/sheet` carries a `new` marker on exactly those lines; restart the app → both are still there, because `is_new` is a column and not a cache.
- [X] T071 [US5] ~~Implement `POST /alerts/{match_id}/ack`.~~ **WITHDRAWN (amendment 3.)** Written, then removed. An acknowledgement that made a line stop looking new was a way to make a recalled item quieter without pulling it — and "new" now means one thing only: *this was not on yesterday's run*. That is a fact about two runs, and no click should be able to edit it. The line stops being new tomorrow, by itself. **Verify**: `grep -rc 'acknowledge_alert' pullsheet/` → 0, and `pytest tests/unit/test_clearing_audit.py::test_no_status_is_ever_updated_on_a_match -v` → passes.
- [X] T072 [US5] Write ~~`tests/integration/test_monitor.py`~~ **`tests/integration/test_new_since_last_run.py`** covering the US5 acceptance scenarios, including persistence across a restart. **Verify**: `pytest tests/integration/test_new_since_last_run.py -v` → 8 pass: the first run flags nothing; an unchanged shelf is still recorded with zero new lines; an arriving recalled item is flagged and nothing else is; a recall published overnight is new on the next `rematch` run; a carried-over item is never new; an item that drops off one export and returns is not new; `is_new` survives a restart; and it is never edited after the run is written.
...
else in the phase follows the section order: matcher → ingestion → sheet → manual floor → proof.
...
files; hand them to three people. T033/T034 (watched folder) and T047/T048 (the manual floor) are all
...
| T007 Inventory fixture | ~50 authored rows carrying every edge case the demo needs | T007a–c, one storage location each (parallel after the header row) |
...
US4 (the status word as the opening frame) → US1 (the drop and the sheet) → US2 → US3, with US5
