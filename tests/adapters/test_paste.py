...
@pytest.mark.parametrize("text", [
    ...
    "🥕🥕🥕",
    "\x00\x01\x02",
    "Site,Item,Qty\nLincoln,chkn strips froz,14\n",     # a whole CSV, pasted by mistake
    "chkn strips froz\ngrnd bf 80/20\nmozz shred lm",
    None,
    12345,
    "  12 chkn strips froz  ",
    "chkn strips froz x 12",
    "chkn strips froz, 12 cases",
    "-1 chkn strips",
    "1e400 chkn strips",
    "\r\n\r\nchkn strips froz\r\n",
])
...
def test_a_leading_count_is_parsed(adapter):
    r = next(iter(adapter.read("12 chkn strips froz")))
    assert r.quantity == 12.0
    assert r.raw_description == "chkn strips froz"
    assert "quantity" not in r.unpopulated


def test_a_trailing_count_is_parsed(adapter):
    r = next(iter(adapter.read("chkn strips froz x 12")))
    assert r.quantity == 12.0
    assert r.raw_description == "chkn strips froz"
...
def test_no_quantity_stays_none_and_is_flagged(adapter):
    ...
    r = next(iter(adapter.read("chkn strips froz")))
...
def test_a_pasted_csv_still_produces_records(adapter):
    ...
    assert "chkn strips froz" in records[1].raw_description
