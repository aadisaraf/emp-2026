...
- [X] T050 [US1] Write `tests/integration/test_abbreviations.py` (SC-005) — every seeded correspondence in `expected_matches.json` appears on the sheet as PULL or HELD. A seeded pair that is **absent** is a failure; a seeded pair appearing as HELD rather than PULL is not. **Verify**: `pytest tests/integration/test_abbreviations.py -v` → all seeded pairs present with the expected evidence kind.
